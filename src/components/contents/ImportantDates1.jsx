...
const ImportantDates1 = () => {
  const dates = [
    ...
    {
      event: "Copyright Submission",
      date: "Starts from 20 Nov 2024",
      remarks: "",
    },
  ];
        ...
        <div className="space-y-2">
          {dates.map((item, index) => (
            ...
            >
              ...
              <p className="text-blue-900 mb-1">Date: {item.date}</p>{" "}
              {/* Dark Blue for Date */}
              {item.remarks && (
                <p className="text-gray-700 text-xs">Remarks: {item.remarks}</p>
              )}
            </div>
          ))}
        </div>
